...
- (void)setup {
    ...
    self.blurEffectView = self.glassEffectView;
    ...
    // --- Social Buttons (Telegram/Discord) ---
    CGFloat socialSize = 32;
    self.telegramButton = [UIButton buttonWithType:UIButtonTypeCustom];
    self.telegramButton.frame = CGRectMake(contentStartX, CGRectGetHeight(self.bounds) - socialSize - 12, socialSize, socialSize);
    self.telegramButton.backgroundColor = [UIColor colorWithWhite:1.0 alpha:0.1];
    self.telegramButton.layer.cornerRadius = 8;
    if (@available(iOS 13.0, *)) {
        [self.telegramButton setImage:[UIImage systemImageNamed:@"paperplane.fill"] forState:UIControlStateNormal];
    }
    [self.telegramButton addTarget:self action:@selector(socialButtonTapped:) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:self.telegramButton];

    self.discordButton = [UIButton buttonWithType:UIButtonTypeCustom];
    self.discordButton.frame = CGRectMake(contentStartX + socialSize + 8, CGRectGetHeight(self.bounds) - socialSize - 12, socialSize, socialSize);
    self.discordButton.backgroundColor = [UIColor colorWithWhite:1.0 alpha:0.1];
    self.discordButton.layer.cornerRadius = 8;
    if (@available(iOS 13.0, *)) {
        [self.discordButton setImage:[UIImage systemImageNamed:@"bubble.left.and.bubble.right.fill"] forState:UIControlStateNormal];
    }
    [self.discordButton addTarget:self action:@selector(socialButtonTapped:) forControlEvents:UIControlEventTouchUpInside];
...
// --- Implementation of methods from .h ---
...
- (void)setMenuSubtitle:(NSString *)subtitle { /* Optional */ }
